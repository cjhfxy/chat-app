<template>
	<view>
          ...
          opacity: transparentValue,
			...
			<view class="header_content">
				<view class="header_left_box">
					<slot name="left">
						<view v-if="back || $slots.left || home" class="header_left_info" :class="[
                isTwoBtn ? 'header_btnMongol' : '',
                isWhite && isTwoBtn ? 'header_colorWhite_btnMongol' : '',
              ]">
							<view v-if="back && !firstPage" class="header_left_back"
								:class="[isTwoBtn ? 'header_btnMongol_left_back' : '']" @click="onBackPage">
								<u-icon name="arrow-left" :color="isWhite?'#fff':'#212121'"></u-icon>
							</view>
							<text v-if="isTwoBtn" class="header_left_line"
								:class="[isWhite ? 'header_colorWhite_left_line' : '']" />
							<view v-if="(firstPage && back) || home" class="header_left_home"
								:class="[isTwoBtn ? 'header_btnMongol_left_home' : '']" @click="onBackHome">
								<u-icon name="home-fill" :color="isWhite?'#fff':'#212121'"></u-icon>
							</view>
						</view>
					</slot>
					<view v-if="!titleCenter && ($slots.default || navTitle)" class="header_title"
				...
				<view v-if="titleCenter && ($slots.default || navTitle)" class="header_title header_title_center"
					:style="{ color: navFontColor }">
					<slot>
						<text :style="{ color: navFontColor }">{{ navTitle }}</text>
					</slot>
		...
		<view v-if="type == 'transparentFixed'" class="header_transparentFixed header_fixed" :style="{
        paddingTop: statusBarHeight + 'px',
			...
			<view class="header_content">
				<view class="header_left_box">
					<slot name="transparentFixedLeft">
						<view v-if="back || $slots.left || home"
							class="header_left_info header_transparentFixed_left_info" :class="[
                isWhite ? 'header_transparentFixed_colorWhite_left_info' : '',
					...
					<view v-if="!titleCenter && (navTitle || $slots.transparentFixed)" class="header_title"
				...
				<view v-if="titleCenter && (navTitle || $slots.transparentFixed)"
					class="header_title header_title_center" :style="{ color: navTransparentFixedFontColor }">
					<slot name="transparentFixed">
						<text :style="{ color: navTransparentFixedFontColor }">{{
              navTitle
            }}</text>
