<template>
	<view class="container">
		<nav-bar :back="1" title="修改提现密码" />
		<view v-if="show" class="set-wrap">
			<view v-if="isPayPasswordSet" class="item">
				<view class="label">
					当前密码
				</view>
				<view class="value">
			...
			<view class="button" @tap="submit">{{isPayPasswordSet?'修改密码':'设置密码'}}</view>
		</view>
		<m-loading v-else />
	</view>
</template>
	...
	export default {
		...
		onShow() {
		  this.$store.dispatch('user/getUserInfo').then(res => {
		    this.isPayPasswordSet = res.isPayPasswordSet
				this.show = true
		  })
		},
		methods: {
			...
			submit() {
				if (this.isPayPasswordSet && !this.oldPassword) {
				  uni.showToast({
				    title: '请输入当前密码',
				    icon: 'none'
				  })
				...
				if (!this.newPassword) {
				  uni.showToast({
				    title: this.isPayPasswordSet ? '请输入新登录密码' :  '请输入登录密码',
				    icon: 'none'
				  })
				  return
				}
